...
router.post('/register', authenticate, validate(validators.register), async (req: Request<{}, {}, RegisterRequest>, res: Response) => {
...
router.delete('/record/:name', authenticate, async (req: Request<{ name: string }>, res: Response) => {
  const { name } = req.params;
