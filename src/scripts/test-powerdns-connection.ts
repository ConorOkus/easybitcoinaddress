...
async function testPowerDNSConnection(): Promise<void> {
  ...
  try {
    const response = await axios.get(`${config.powerdns.apiUrl}/api/v1/servers/${config.powerdns.serverId}`, {
      headers: {
        'X-API-Key': config.powerdns.apiKey,
        'Content-Type': 'application/json'
      },
  ...
  try {
    const response = await axios.get(`${config.powerdns.apiUrl}/api/v1/servers/${config.powerdns.serverId}/zones/${config.dns.zone}`, {
      headers: {
        'X-API-Key': config.powerdns.apiKey,
        'Content-Type': 'application/json'
      },
  ...
  try {
    ...
    await axios.patch(`${config.powerdns.apiUrl}/api/v1/servers/${config.powerdns.serverId}/zones/${config.dns.zone}`, rrsets, {
      headers: {
        'X-API-Key': config.powerdns.apiKey,
        'Content-Type': 'application/json'
      },
    ...
    try {
      const zoneResponse = await axios.get(`${config.powerdns.apiUrl}/api/v1/servers/${config.powerdns.serverId}/zones/${config.dns.zone}`, {
        headers: {
          'X-API-Key': config.powerdns.apiKey,
          'Content-Type': 'application/json'
        },
      ...
      const foundRecord = zone.rrsets.find((rr: any) => 
        rr.name.toLowerCase() === normalizedTestName && rr.type === 'TXT'
      );
      ...
      if (foundRecord) {
        ...
        results.push({
          ...
          details: {
            ...
            availableRecords: zone.rrsets.filter((rr: any) => rr.type === 'TXT').map((rr: any) => rr.name)
          }
        });
      }
    ...
    try {
      ...
      await axios.patch(`${config.powerdns.apiUrl}/api/v1/servers/${config.powerdns.serverId}/zones/${config.dns.zone}`, deleteRrsets, {
        headers: {
          'X-API-Key': config.powerdns.apiKey,
          'Content-Type': 'application/json'
        },
