...
describe('Records Routes', () => {
  ...
  describe('DELETE /record/:name', () => {
    it('should delete an existing record', async () => {
      ...
      mockPowerDNS.getTXTRecord.mockResolvedValueOnce(mockRecord);
      mockPowerDNS.deleteTXTRecord.mockResolvedValueOnce(true);
      ...
      expect(response.body).toEqual({
        ...
        fqdn: mockRecord.fqdn,
      });
      expect(mockPowerDNS.getTXTRecord).toHaveBeenCalledWith('conor');
      expect(mockPowerDNS.deleteTXTRecord).toHaveBeenCalledWith('conor');
    });
    ...
    it('should return 404 for non-existent record', async () => {
      ...
      expect(mockPowerDNS.deleteTXTRecord).not.toHaveBeenCalled();
    });

    it('should return 401 without authentication', async () => {
      const response = await request(app).delete('/record/conor');
    ...
    it('should handle PowerDNS service errors during retrieval', async () => {
      mockPowerDNS.getTXTRecord.mockRejectedValueOnce(new Error('PowerDNS API error'));
    ...
    it('should handle PowerDNS service errors during deletion', async () => {
      const mockRecord: RecordResponse = {
        fqdn: 'conor.user._bitcoin-payment.easybitcoinaddress.me.',
        uri: 'bitcoin:bc1qexample...',
      };
      ...
      mockPowerDNS.getTXTRecord.mockResolvedValueOnce(mockRecord);
      mockPowerDNS.deleteTXTRecord.mockRejectedValueOnce(new Error('PowerDNS deletion failed'));
      ...
      expect(response.status).toBe(500);
      expect(response.body).toEqual({
        error: 'PowerDNS deletion failed',
      });
    });
  });
});
